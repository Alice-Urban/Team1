		...
		protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			String bottone = request.getParameter("bottone2");
				 ...
				 try {
					...
					session.setAttribute("Errore","Siamo spiacenti, si Ë verificato un errore durante l'eliminazione dell'utente.");
					richiesta.forward(request, response);
				}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (utenteDaInserire!= null) {
				...
				session.setAttribute("Errore", "Utente gi‡ inserito!");
				richiesta.forward(request,response);
		...
		sql.getMessage();
		richiesta = request.getRequestDispatcher("Error.jsp");
		session.setAttribute("Errore","Si Ë verificato un errore nella registrazione dei dati.");
		richiesta.forward(request, response);
